...
def get_default_configs(service_name: str, version_number: str) -> dict[str, Any]:
    config_schema = get_config_schema(service_name=service_name, version_number=version_number)
    config = {}
...
def get_config_from_manifest(manifest: IntegrationManifest, 
                             event_mapping: dict[str, str]) -> dict[str, Any]:
    config = {}
    if manifest.api.rest:
        ...
        if rest_manifest.consumes:
            for service_name, details in rest_manifest.consumes.items():
                config[details.config] = f"Please fill with api url for service {service_name}"
    ...
    if manifest.api.events:
        ...
        for event_type, details in events.items():
            if event_type not in event_mapping: 
                raise Exception
            config[details.config] = event_type
            config[details.topic_config] = event_mapping[event_type]

    if manifest.storage:
        if manifest.storage.s3:
            for s3details in manifest.storage.s3:
                config[s3details.config] = s3details.bucket
    ...
    if manifest.extra_config:
        for config_name, details in manifest.extra_config.items():
            config[config_name] = details.value or f'<Please fill - {details.value_instructions}>'
...
def generate_config_for_service(service_short_name: str):
    ...
    config = get_default_configs(service_name=manifest.name,
                                 version_number=manifest.version)
    event_mapping = get_event_type_mapping()
    manifest_config = get_config_from_manifest(manifest=manifest, event_mapping=event_mapping)
